...
//Parts to change for OS and IW *** Change to your default chain texture ***
...
Linking (integer n, key Target, key Texture )
{
...
llLockGuardObey()
{
    ...
    do
    {
        ...
        if( nReturn == 1 )//lets link the chain but we need to find the destination first
        {
            ...
            Linking(n , lgkTarget, lgkTexture);// format -  prim link number, target prim, texture to use
        }
